...
#include "Evolution/Systems/ScalarWave/ShellCoordinates/TagsDeclarations.hpp"
...
struct PhiR : db::SimpleTag {
  using type = tnsr::i<DataVector, 1, Frame::Inertial>;
  static std::string name() noexcept { return "PhiR"; }
};

template <size_t Dim>
struct PhiA : db::SimpleTag {
  using type = tnsr::i<DataVector, Dim - 1, Frame::Inertial>;
  static std::string name() noexcept { return "PhiA"; }
};
...
struct EvolvedFieldsFromCharacteristicFields : db::SimpleTag {
  using type = Variables<tmpl::list<Psi, Pi, PhiR, PhiA<Dim>>>;
