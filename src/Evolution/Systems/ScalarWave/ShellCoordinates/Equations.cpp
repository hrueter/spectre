...
#include "Evolution/Systems/ScalarWave/ShellCoordinates/System.hpp"
#include "Evolution/Systems/ScalarWave/ShellCoordinates/Tags.hpp"  // IWYU pragma: keep
...
void ComputeNormalDotFluxes<Dim>::apply(
    ...
    const Scalar<DataVector>& pi, const Scalar<DataVector>& phiR,
    const tnsr::i<DataVector, Dim - 1, Frame::Inertial>& phiA,
...
void UpwindFlux<Dim>::package_data(
    ...
    const Scalar<DataVector>& normal_dot_flux_phiR,
    const tnsr::i<DataVector, Dim - 1, Frame::Inertial>& normal_dot_flux_phiA,
    const Scalar<DataVector>& pi,
  ...
  get<::Tags::NormalDotFlux<PhiR>>(*packaged_data) = normal_dot_flux_phiR;
  get<::Tags::NormalDotFlux<PhiA<Dim>>>(*packaged_data) = normal_dot_flux_phiA;
...
void UpwindFlux<Dim>::operator()(
    ...
    const gsl::not_null<Scalar<DataVector>*> phiR_normal_dot_numerical_flux,
    const gsl::not_null<tnsr::i<DataVector, Dim - 1, Frame::Inertial>*>
        phiA_normal_dot_numerical_flux,
    const gsl::not_null<Scalar<DataVector>*> psi_normal_dot_numerical_flux,
    const Scalar<DataVector>& normal_dot_flux_pi_interior,
    const Scalar<DataVector>& normal_dot_flux_phiR_interior,
    const tnsr::i<DataVector, Dim - 1, Frame::Inertial>&
        normal_dot_flux_phiA_interior,
    ...
    const Scalar<DataVector>& minus_normal_dot_flux_phiR_exterior,
    const tnsr::i<DataVector, Dim - 1, Frame::Inertial>&
        minus_normal_dot_flux_phiA_exterior,
...
using derivative_tags =
    typename ScalarWave::ShellCoordinates::System<Dim>::gradients_tags;

template <size_t Dim>
using variables_tags = typename ScalarWave::ShellCoordinates::System<
    Dim>::variables_tag::tags_list;
...
#define INSTANTIATION(_, data)                                               \
  template class ScalarWave::ShellCoordinates::ComputeDuDt<DIM(data)>;       \
  template class ScalarWave::ShellCoordinates::ComputeNormalDotFluxes<DIM(   \
      data)>;                                                                \
  template class ScalarWave::ShellCoordinates::UpwindFlux<DIM(data)>;        \
  template Variables<                                                        \
