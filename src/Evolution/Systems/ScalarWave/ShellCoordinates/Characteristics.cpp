...
#include "Evolution/Systems/ScalarWave/ShellCoordinates/Characteristics.hpp"
...
void evolved_fields_from_characteristic_fields(
    const gsl::not_null<Variables<tmpl::list<Psi, Pi, PhiR, PhiA<Dim>>>*>
        evolved_fields,
...
template <size_t Dim>
Variables<tmpl::list<Psi, Pi, PhiR, PhiA<Dim>>>
evolved_fields_from_characteristic_fields(
    ...
    const tnsr::i<DataVector, Dim, Frame::Inertial>&
        unit_normal_one_form) noexcept {
  Variables<tmpl::list<Psi, Pi, PhiR, PhiA<Dim>>> evolved_fields(
...
#define INSTANTIATE(_, data)                                                   \
  template void ScalarWave::ShellCoordinates::characteristic_speeds(           \
      const gsl::not_null<std::array<DataVector, 4>*> char_speeds,             \
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&                   \
          unit_normal_one_form) noexcept;                                      \
  template std::array<DataVector, 4>                                           \
  ScalarWave::ShellCoordinates::characteristic_speeds(                         \
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&                   \
          unit_normal_one_form) noexcept;                                      \
  template struct ScalarWave::ShellCoordinates::CharacteristicSpeedsCompute<   \
      DIM(data)>;                                                              \
  template void ScalarWave::ShellCoordinates::characteristic_fields(           \
      const gsl::not_null<Variables<                                           \
          tmpl::list<ScalarWave::ShellCoordinates::Tags::VPsi,                 \
                     ScalarWave::ShellCoordinates::Tags::VZero<DIM(data)>,     \
                     ScalarWave::ShellCoordinates::Tags::VPlus,                \
                     ScalarWave::ShellCoordinates::Tags::VMinus>>*>            \
          char_fields,                                                         \
      const Scalar<DataVector>& gamma_2, const Scalar<DataVector>& psi,        \
      const Scalar<DataVector>& pi, const Scalar<DataVector>& phiR,            \
      const tnsr::i<DataVector, DIM(data) - 1, Frame::Inertial>& phiA,         \
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&                   \
          unit_normal_one_form) noexcept;                                      \
  template Variables<                                                          \
      tmpl::list<ScalarWave::ShellCoordinates::Tags::VPsi,                     \
                 ...
                 ScalarWave::ShellCoordinates::Tags::VPlus,                    \
                 ScalarWave::ShellCoordinates::Tags::VMinus>>                  \
  ScalarWave::ShellCoordinates::characteristic_fields(                         \
      const Scalar<DataVector>& gamma_2, const Scalar<DataVector>& psi,        \
      const Scalar<DataVector>& pi, const Scalar<DataVector>& phiR,            \
      const tnsr::i<DataVector, DIM(data) - 1, Frame::Inertial>& phiA const    \
          tnsr::i<DataVector, DIM(data), Frame::Inertial>&                     \
              unit_normal_one_form) noexcept;                                  \
  template struct ScalarWave::ShellCoordinates::CharacteristicFieldsCompute<   \
      DIM(data)>;                                                              \
  template void                                                                \
  ScalarWave::ShellCoordinates::evolved_fields_from_characteristic_fields(     \
      const gsl::not_null<Variables<tmpl::list<                                \
          ScalarWave::ShellCoordinates::Psi, ScalarWave::ShellCoordinates::Pi, \
          ScalarWave::ShellCoordinates::PhiR,                                  \
          ScalarWave::ShellCoordinates::PhiA<DIM(data)>>>*>                    \
          evolved_fields,                                                      \
      const Scalar<DataVector>& gamma_2, const Scalar<DataVector>& v_psi,      \
      ...
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&                   \
          unit_normal_one_form) noexcept;                                      \
  template Variables <                                                         \
      tmpl::list<ScalarWave::ShellCoordinates::Psi,                            \
                 ScalarWave::ShellCoordinates::Pi,                             \
                 ScalarWave::ShellCoordinates::PhiR,                           \
                 ScalarWave::ShellCoordinates::PhiA<DIM(data)>>                \
      ScalarWave::ShellCoordinates::evolved_fields_from_characteristic_fields( \
          const Scalar<DataVector>& gamma_2, const Scalar<DataVector>& v_psi,  \
          ...
          const tnsr::i<DataVector, DIM(data), Frame::Inertial>&               \
              unit_normal_one_form) noexcept;                                  \
  template struct ScalarWave::ShellCoordinates::                               \
