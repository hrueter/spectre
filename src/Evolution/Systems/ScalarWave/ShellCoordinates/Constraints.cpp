...
#include "Evolution/Systems/ScalarWave/ShellCoordinates/Constraints.hpp"
...
#define INSTANTIATE(_, data)                                                  \
  ...
  ScalarWave::ShellCoordinates::one_index_constraint(                         \
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&,                 \
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&) noexcept;       \
  template void ScalarWave::ShellCoordinates::one_index_constraint(           \
      const gsl::not_null<tnsr::i<DataVector, DIM(data), Frame::Inertial>*>,  \
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&,                 \
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&) noexcept;       \
  template tnsr::ij<DataVector, DIM(data), Frame::Inertial>                   \
  ScalarWave::ShellCoordinates::two_index_constraint(                         \
      const tnsr::ij<DataVector, DIM(data), Frame::Inertial>&) noexcept;      \
  template void ScalarWave::ShellCoordinates::two_index_constraint(           \
