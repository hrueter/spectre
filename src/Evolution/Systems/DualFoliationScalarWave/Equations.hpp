 ...
 * first-order dual foliation scalar wave system.
 *
 * The evolution equations for the first-order dual foliation scalar wave
 * system are given by Eq. (23) of \cite Gasperin2019rjg:
 * \f{align}
 * \partial_t\Psi = & -\Pi \\
 ...
 * auxiliary variable.
 *
 * ToDo: Implement version with regularized fields. Implement regularized
 * sources for n=2 height functions employing l'Hopital's rule.
 * Implement good, bad and ugly wave equations. (1909.11749)
 * \cite Hilditch2016xzh
 * fixme: turn ToDo into a github issue
 */
template <size_t Dim>
struct ComputeDuDt {
 ...
 * DualFoliationScalarWave::ComputeNormalDotFluxes
 */
template <size_t Dim>
struct UpwindFlux {
 private:
 ...
 public:
  ...
  // `()` operator.
  using package_tags = tmpl::list<Tags::NormalDotFlux<::ScalarWave::Pi>,
                                  Tags::NormalDotFlux<::ScalarWave::Phi<Dim>>,
                                  ::ScalarWave::Pi, NormalTimesFluxPi>;
  ...
  using argument_tags =
      tmpl::list<Tags::NormalDotFlux<::ScalarWave::Pi>,
                 Tags::NormalDotFlux<::ScalarWave::Phi<Dim>>, ::ScalarWave::Pi,
                 Tags::Normalized<Tags::UnnormalizedFaceNormal<Dim>>>;
