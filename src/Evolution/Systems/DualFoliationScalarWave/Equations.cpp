...
void UpwindFlux<Dim>::package_data(
  ...
  get<::ScalarWave::Pi>(*packaged_data) = pi;
  get<Tags::NormalDotFlux<::ScalarWave::Pi>>(*packaged_data) =
      normal_dot_flux_pi;
  get<Tags::NormalDotFlux<::ScalarWave::Phi<Dim>>>(*packaged_data) =
      normal_dot_flux_phi;
...
using derivative_tags =
    typename DualFoliationScalarWave::System<Dim>::gradients_tags;

template <size_t Dim>
using variables_tags =
    typename DualFoliationScalarWave::System<Dim>::variables_tag::tags_list;
...
#define INSTANTIATION(_, data)                                               \
  template class DualFoliationScalarWave::ComputeDuDt<DIM(data)>;            \
  template class DualFoliationScalarWave::ComputeNormalDotFluxes<DIM(data)>; \
  template class DualFoliationScalarWave::UpwindFlux<DIM(data)>;             \
  template Variables<                                                        \
