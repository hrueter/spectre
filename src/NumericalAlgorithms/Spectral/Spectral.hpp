...
/*!
 * \brief %Matrix for exponential filtering
 *
 * \details Filters out the high modes applying
 * \f$\mathcal{V}^{-1}\cdot\mathrm{diag}(exp(0),...,\exp(-\alpha
 * (n/(N-1))^s),...,\exp(-\alpha)\cdot\mathcal{V}\f$ to the nodal coefficients,
 * where \f$n\f$ is the order of the basis polynomial, \f$N\f$ the number of
 * points and \f$\mathcal{V}\f$ is the Vandermonde matrix computed in
 * `spectral_to_grid_points_matrix(size_t)`.
 * For discussion refer to \cite HesthavenWarburton Ch. 5.3 ff.
 *
 * \param num_points The number of collocation points
 * \param alpha damping factor in the exponent
 * \param s damping exponent
 *
 ...
 */
template <Basis BasisType, Quadrature QuadratureType>
Matrix exponential_filter_matrix(size_t num_points, double alpha,
                                 size_t s) noexcept;

/*!
 * \brief %Matrix for exponential filtering on one-dimensional mesh
 *
 * \param mesh The mesh on which the filter is applied
 * \param alpha damping factor in the exponent
 * \param s damping exponent
 *
 * \return Filter matrix
 *
 * \see exponential_filter_matrix(size_t, double, size_t)
 */
Matrix exponential_filter_matrix(const Mesh<1>& mesh, double alpha,
                                 size_t s) noexcept;

/*!
 * \brief  create the cache holding the %Matrix for exponential filtering
 *
 * \see exponential_filter_matrix(size_t, double, size_t)
 *
 * \param mesh The mesh on which the filter is applied
 ...
 */
void exponential_filter_matrix_make_cache(const Mesh<1>& mesh, double alpha,
                                          size_t s) noexcept;

template <Basis BasisType, Quadrature QuadratureType>
void exponential_filter_matrix_make_cache(size_t num_points, double alpha,
                                          size_t s) noexcept;

/*!
 * \brief get the %Matrix generated by exponential_filter_matrix_make_cache
 * \see exponential_filter_matrix_make_cache(size_t, double, size_t)
 * \see exponential_filter_matrix(size_t, double, size_t)
 *
 * \param mesh The mesh on which the filter is applied
 *
 * \return cached filter matrix *
 */
const Matrix& exponential_filter_matrix_get_cached(
    const Mesh<1>& mesh) noexcept;

template <Basis BasisType, Quadrature QuadratureType>
const Matrix& exponential_filter_matrix_get_cached(size_t num_points) noexcept;
