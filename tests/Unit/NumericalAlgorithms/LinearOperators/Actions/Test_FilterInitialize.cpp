...
void test_filter_initialize() {
  ...
  CHECK(filter_with_cached_matrix(var_to_filter, mesh) ==
        filter(var_to_filter, mesh, alpha, s));
}

SPECTRE_TEST_CASE(
    "Unit.Numerical.LinearOperators.Filter.Actions.FilterInitialize",
    "[Unit][NumericalAlgorithms][LinearOperators][Actions]") {
  test_filter_initialize();
}
